...
void c2_plus()
	{
		if(trace_flag == 1)		
			{
				strcpy(trace_1, "c2z.incr.c c2_plus");
				trace_rec_1();
			}
		...
		if((p) && (!p1) && (!p2) && (!p3))
			{
				...
				if(lv_ct > 0)
					{
						for(I=0; I < lv_ct; I++)
							{
								strcpy(a_string, lw_variable[I].lv_name);
								...
								if((ret == 0) && (ret1 == 0))
									{
										...
										pl4 = strcmp(lw_variable[I].lv_type, "I");
										if((pl4 != 0) && (pl1 == 1))
											{
												printf("c2z_incr.c c2_plus E-293 #3d ERROR\n");
												c2_error();
											}
									}
							}
				...
				if(pl1 == 0)
					{
						printf("c2z_incr.c c2_plus E-294 field1 Not Found = %s\n",field1);
						c2_error();
					}
